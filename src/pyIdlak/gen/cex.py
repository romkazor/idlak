...
class CEXParser:
    def __init__(self, cexspec, include_phon_val = True):
        if type(cexspec) == str:
    ...
    def set_conversions(self, cexfreqtable, norm_ints = True):
        ...
        for cexid, val_freqs in cexfreqtable.items():
            if cexid == 'cex000':
                ...
                cexfunc = CexFunc('PhoneVal', 'cex000', None, False)
            else:
                cexfunc = [c for c in self._cexfunctions if c.id == cexid][0]
            ...
            all_int = all(map(_isint, cex_values))
            if all_int:
                cex_values = list(map(int, cex_values))
            else:
                if cexfunc.isinteger:
                    raise ValueError('Non integer value in integer cex feature frequency table: '+ cexid)
            cex_values.sort()
    ...
    def parse(self, xml):
        ...
        for spurt in spurt_iterator:
            ...
            if spurt_id is None:
                ...
                self._untitled_counter += 1
            cex_features[spurt_id] = self._parse_spurt(spurt)
    ...
    def _parse_header(self, xml):
        ...
        # Build a regex for extracting the context from the phone tags
        # get the ctxid for getting the  phone name
        pat = ''
    ...
    def _parse_spurt(self, spurt):
        ...
        for pron in spurt.iter('phon'):
            ...
            cexfeats.append(self._pron_to_feat(pron))
        return cexfeats


    def _pron_to_feat(self, pron):
        """ Converts a pron tag to context features """
        m = self._repat.match(pron.text)
        if m is None:
            self._parse_err = True
            print("Bad phone context string: ", pron.text, file=sys.stderr)
        phone_name = pron.attrib.get('val', 'pau')
        cexs = []
    ...
    def _cex_to_features(self, cex_features):
        ...
        if self._include_phon_val:
            cexids = ['cex000'] + [c.id for c in self._cexfunctions]
        else:
            cexids = [c.id for c in self._cexfunctions]

        spurt_features = []
        for phone_cex in cex_features:
            phone_features = []
            for cex, cexid in zip(phone_cex, cexids):
                conversion = self._conversions[cexid]
                if conversion.mapping:
                    try:
                        val = conversion.mapping[cex]
                    except KeyError:
                        print("Bad unknown value '{0}' for context feature '{1}' in context string: ".
                              format(cex, cexid), ' '.join(phone_cex), file=sys.stderr)
                        val = 0
                else:
                    val = cex
