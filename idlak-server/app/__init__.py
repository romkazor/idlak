...
from flask_migrate import Migrate, init as db_init, migrate as db_migrate
from flask_migrate import upgrade as db_upgrade, show
...
def db_setup():
    with app.app_context():
        if not os.path.isdir('migrations'):
            app.logger.info("The database is being created, to run the " +
                            "server it has to be run again afterwards")
            db_init()
            db_migrate()
            db_upgrade()
            return True
...
# this is the only way to make the initial db setup work since after
# running it, it shuts down the program
if db_setup():
    show()
